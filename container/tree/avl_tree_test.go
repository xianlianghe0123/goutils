...
func checkAvlTree(t *testing.T, avl *AvlTree[int], expect []int, msg ...any) {
	...
	s := slices.Collect(avl.Iter())
	for i := range len(s) - 1 {
		require.True(t, s[i+1] > s[i], msg...)
	}
}

func TestAvlTree_ReadNotExist(t *testing.T) {
	avl := NewAvlTree[int](cmp.Compare)
...
func buildAvlTree(elems []int) *AvlTree[int] {
	avl := NewAvlTree[int](cmp.Compare)
